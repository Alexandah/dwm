...
struct Client {
	...
	int bw, oldbw;
	unsigned int tags;
	int isfixed, isfloating, isurgent, neverfocus, oldstate, isfullscreen, issticky, issteam;
	Client *next;
	Client *snext;
	Monitor *mon;
	Window win;
...
seturgent(Client *c, int urg)
{
	...
	c->isurgent = urg;
	XSetWindowBorder(dpy, c->win, scheme[urg ? SchemeUrgent : SchemeNorm][ColBorder].pixel);
...
unfocus(Client *c, int setfocus)
{
	if (!c)
		return;
	grabbuttons(c, 0);
	XSetWindowBorder(dpy, c->win, scheme[c->isurgent ? SchemeUrgent : SchemeNorm][ColBorder].pixel);
	if (setfocus) {
		XSetInputFocus(dpy, root, RevertToPointerRoot, CurrentTime);
		XDeleteProperty(dpy, root, netatom[NetActiveWindow]);
	}
...
updatewmhints(Client *c)
{
	...
	if ((wmh = XGetWMHints(dpy, c->win))) {
		if (c == selmon->sel && wmh->flags & XUrgencyHint) {
			...
			XSetWindowBorder(dpy, c->win, scheme[c == selmon->sel ? SchemeSel : (c->isurgent ? SchemeUrgent : SchemeNorm)][ColBorder].pixel);
		}
